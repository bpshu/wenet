...
int process(int process_id) {
  ...
  // Send data every 0.04 second
  const float interval = 0.04;
  ...
  for (int start = 0; start < num_sample; start += sample_interval) {
    ...
    VLOG(2) << "Process_id-" << process_id <<": Send " << data.size() << " samples";
  ...
  client.Join();
  VLOG(2) << "Process_id-" << process_id << ": Total latency: " << timer.Elapsed() << "ms.";
  return 0;
}
...
int main(int argc, char* argv[]){
  ...
  //
  int NumofThread = FLAGS_nthreads;
